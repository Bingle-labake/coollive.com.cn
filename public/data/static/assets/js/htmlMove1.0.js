function moveHtml(json) {
	var tag = json.element.children("dl"), width = tag.eq(0).width(), n = 0, fn = true, className = json.className||'on', cur = n, len = tag.length;
...
(function() {
	function scrollTxt(element){
		var c = element.find('li:first'), 
			s = element.next(),
			size = c.height()/s.height(), 
			aH = element.height()/size, 
			a = s.find('i:first').css({'height':aH}),
			maxM = s.height() - aH;
		var starY, now, isMove = 0, top = 0;
		if(size<=1){
			s.hide();
		...
		function _move(){
			...
			c.css('margin-top',-top*size);
		}
	}
	jQuery.scrollTxt = scrollTxt;
})();
