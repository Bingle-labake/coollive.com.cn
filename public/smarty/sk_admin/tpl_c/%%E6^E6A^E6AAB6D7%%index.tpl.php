<?php /* Smarty version 2.6.27, created on 2014-04-14 09:19:25
         compiled from sk_admin/prototype_cate/index.tpl */ ?>
<?php require_once(SMARTY_CORE_DIR . 'core.load_plugins.php');
smarty_core_load_plugins(array('plugins' => array(array('modifier', 'default', 'sk_admin/prototype_cate/index.tpl', 12, false),)), $this); ?>
...
<?php $_from = $this->_tpl_vars['prototype_cate_list']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }if (count($_from)):
    foreach ($_from as $this->_tpl_vars['prototype_cate']):
?>
 <tr id="tr_<?php echo $this->_tpl_vars['prototype_cate']['id']; ?>
">
   <td>&nbsp;<?php echo $this->_tpl_vars['prototype_cate']['id']; ?>
</td>
   <?php if ($this->_tpl_vars['prototype_cate']['level'] == 1): ?>
   <td width="160"><?php echo $this->_tpl_vars['prototype_cate']['name']; ?>
</td>
   <td width="160">&nbsp;</td>
   <?php endif; ?>
   
   <?php if ($this->_tpl_vars['prototype_cate']['level'] == 2): ?>
   <td width="160">&nbsp;</td>
   <td width="160"><?php echo $this->_tpl_vars['prototype_cate']['name']; ?>
[<?php echo $this->_tpl_vars['prototype_cate']['parent_name']; ?>
]</td>
   <?php endif; ?>
   
   <td align="center">
   <?php $_from = $this->_tpl_vars['prototype_cate']['action_list']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }if (count($_from)):
    foreach ($_from as $this->_tpl_vars['action']):
?>
       <?php if ($this->_tpl_vars['action']['ajax']): ?>
       <a href="javascript:void(0);" class="prototype_cate_<?php echo $this->_tpl_vars['action']['ajax']['act']; ?>
...
$('.prototype_cate_remove').live('click', function() {
	var parent = $(this).parent();
	var _data = $(this).attr("data");	
	_data = eval("("+_data+")");
    if(_data.act == "remove" && confirm("是否确定删除？")) {
		post_api(ADMIN_FOLDER+"/prototype_cate/remove?re_type=json", {id:_data.id}, function(res) {
			if(res.code>0) {
				$('#tr_'+_data.id).remove();
			}else {
				try {
...
function setViews(bid,views,elm){
	...
	if(count!=null&&count!="undefined")
	{
		$.ajax({
		type: "post",
		url: ADMIN_FOLDER+"/prototype_cate/setviews",
