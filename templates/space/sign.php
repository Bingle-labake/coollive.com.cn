...
function UploadShareImg(){
	...
	var colid = $("#colid").val();
	$.ajaxFileUpload({
		url:"/space/sign/upload?colid="+colid,
		secureuri:false,
		...
		<div class="block_form">
			<div class="b_video_info">
				...
				<div class="b_video_form b_baoming_form">
					...
					<!--<dl>
						<dt>参赛形式：</dt>
						<dd> <em class="b_video_select">劲舞团</em>
							<input type="hidden" value="劲舞团" />
							<span class="cRed">您已报名过单人舞参赛形式，请选择其他形式</span> </dd>
					</dl>-->
					<dl>
						<dt>参赛形式：</dt>
						<dd>
							<span id="entryform" style="display:block;width: 300px;line-height: 26px;border:1px solid #ccc;padding-left: 10px;">
							<?php if (!empty($sign['entryform'])) { ?><?php echo @$sign['entryform']; ?><?php } else { ?>--请选择参赛形式--<?php } ?>
							</span>
					...
					<dl>
						<dt>参赛名称：</dt>
						<dd>
							<span id="entryname" style="display:block;width: 300px;line-height: 26px;border:1px solid #ccc;padding-left: 10px;"></span>
							<span style="display:none;color:red" id="tip_entryname"></span>
						</dd>
                        <input type="hidden" name="entryname" id="gid" value="" />
					</dl>
					-->
					...
					<dl>
						<dt>所属团队：</dt>
						<dd>
							<input type="text" name="team" id="team" class="b_bm_input" value="<?php echo @$sign['team']; ?>" />
							<span> 请自行填写团队名称，单人参赛不用填写 </span> 
							<span style="display:none;color:red" id="tip_team"></span>
						</dd>
					</dl>
...
<div class="pub-downmenu hide" id="pub-downmenu">
    <?php $type = json_decode($game['type'],true); foreach ($type as $va) { ?>
     <li data-val="<?php echo $va; ?>"><?php echo $entryform[$va]; ?></li>
     <?php } ?>
</div>
