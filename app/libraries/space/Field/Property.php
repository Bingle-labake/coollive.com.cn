 ...
 * @filesource	svn://www.dayrui.net/v2/mall/libraries/Field/Property.php
 */
	...
	public function input($cname, $name, $cfg, $value = NULL) {
		...
		if (isset($cfg['option']) && $cfg['option']) { // 默认属性选项
			foreach ($cfg['option'] as $i => $t) {
				...
				$str.= lang('my-52').'：<input type="text" class="input-text" style="width:140px;" value="'.$t['name'].'" name="data['.$name.']['.$i.'][name]">&nbsp;&nbsp;';
				$str.= lang('my-53').'：';
				switch ($t['type']) {
					case 1:
						$v = $value[$i]['value'] ? $value[$i]['value'] : $t['value'];
						$str.= '<input type="text" class="input-text" style="width:300px;" value="'.$v.'" name="data['.$name.']['.$i.'][value]" />';
						break;
					case 2:
						$v = @explode(',', $t['value']);
						$str.= '<select name="data['.$name.']['.$i.'][value]">';
		...
		if ($value) {
			foreach ($value as $k => $t) {
				if ($k > $i) {
					...
					$str.= lang('my-52').'：<input type="text" class="input-text" style="width:140px;" value="'.$t['name'].'" name="data['.$name.']['.$k.'][name]">&nbsp;&nbsp;';
					$str.= lang('my-53').'：';
					$str.= '<input type="text" class="input-text" style="width:300px;" value="'.$t['value'].'" name="data['.$name.']['.$k.'][value]" />';
		...
		$str.= '	<a href="javascript:;" onClick="dr_add_property_'.$name.'()">'.lang('my-54').'</a>';
		$str.= '</div>';
		...
		function dr_add_property_'.$name.'() {
			...
			html+= "'.lang('my-52').'：<input type=\"text\" class=\"input-text\" style=\"width:140px;\" value=\"\" name=\"data['.$name.']["+id+"][name]\">&nbsp;&nbsp;";
			html+= "'.lang('my-53').'：<input type=\"text\" class=\"input-text\" style=\"width:300px;\" value=\"\" name=\"data['.$name.']["+id+"][value]\">&nbsp;&nbsp;";
