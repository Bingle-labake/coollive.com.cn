...
*/
class Gold_model extends K_Model {
	private $model_data = '';
	 ...
	 * 积分策略检测
	 * @param int $uid  用户id
	 * @param string $act 积分动作
	 * @param int $record_time 动作时间
	 * @return boolean
	 */
	public function check_credit_rule($uid,$act,$record_time = 0){
		$update_credit = false;
		$rule = $this->get_rule($act);
		if (empty($uid) || empty($rule)) {
			return false;
		}else {
			switch ($rule['cycle']) {
				case 0://不限
					...
					break;
				case 1://一次
					$count = $this->get_logs_count($uid,$act);
					$update_credit = empty($count) ? true : false;
					break;
				case 2://每天
					$count = $this->get_logs_count($uid,$act,strtotime(date('Y-m-d 00:00:00')));
					if (empty($count) || $count<$rule['rate']) {
						$update_credit = true;
					}else {
						$update_credit = false;
					}
