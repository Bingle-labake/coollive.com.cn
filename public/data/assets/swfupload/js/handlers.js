...
function uploadSuccess(file, serverData) {
	try {
		...
		progress.toggleCancel(false);
		//alert (serverData.substring(0, 7));
		addImage(serverData.pic_url);
	} catch (ex) {
		this.debug(ex);
	}
}
...
function uploadError(file, errorCode, message) {
	var imageFileError =  "http://img2.saiku.com.cn/static/assets/swfupload/images/error.gif";
	try {
		var progress = new FileProgress(file, this.customSettings.progressTarget);
		progress.setError();
		progress.toggleCancel(false);
